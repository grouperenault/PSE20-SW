...
void affichageInidicateurPerf(Etat_Global &etatLocal){
  if (etatLocal.etatNEMA == HIGH) {
    ...
    if (etatLocal.carMot == 0) {textLCD("x",0,0);}
    else {textLCD("o",0,0);}
  }
}
...
void splashLCD() {
  ...
  lcd.setCursor(14, 1); lcd.write(byte(0));
...
void menuLCD(Etat_Global &etatLocal) {
    ...
    if (etatLocal.displayAlarme == alarmNone) {
      ...
      if (etatLocal.affichageEnCours == menuPrincipal) {
        textLCD("Seringue     mm",1,0);
        textLCD("Debit      ml/h",1,1);
        ...
        if (etatLocal.choixEnCours == choixSeringue) { textLCD(">",0, 0);}
        if (etatLocal.choixEnCours == choixDebit) { textLCD(">",0, 1);}
      } else {

        // Sélection de la seringue
        if (etatLocal.affichageEnCours == reglSeringue) {
          textLCD("Select Seringue",1,0);
          textLCD(">",1,1);
      
          textLCD(etatLocal.seringueList[etatLocal.positionSer],2,1);
          lcd.setCursor(2, 1);
...
void affichageAlarme(Etat_Global &etatLocal) {
  if (existeAlarme(etatLocal)) {
    ...
    if (millis() - etatLocal.alarmeLastDisplay > timerBoucleAlarme ) {
      ...
      } else if (etatLocal.alarmeBatterie && etatLocal.displayAlarme == alarmBatterie) {
        messageAlarme = "DEFAUT BATTERIE";
      } else if (etatLocal.alarmeFinCourse && etatLocal.displayAlarme == alarmFinCourse) {
        messageAlarme = "FIN SERINGUE   ";
      } else if (etatLocal.alarmeDixPourCent && etatLocal.displayAlarme == alarmDixPourCent) {
        messageAlarme = "10% RESTANT    ";
      } else if (etatLocal.alarmeSecteur && etatLocal.displayAlarme == alarmSecteur) {
        messageAlarme = "DEFAUT SECTEUR ";  
      } 
      ...
      else if (etatLocal.displayAlarme == alarmBatterie) {etatLocal.displayAlarme = alarmFinCourse;}
      else if (etatLocal.displayAlarme == alarmFinCourse) {etatLocal.displayAlarme = alarmDixPourCent;}
      else if (etatLocal.displayAlarme == alarmDixPourCent) {etatLocal.displayAlarme = alarmSecteur;}
      else if (etatLocal.displayAlarme == alarmSecteur) {etatLocal.displayAlarme = alarmOcclusion;}
