 ...
 */
      struct Alerte {
        ...
        int      volume        = 0; // fréquence de la sortie buzzer de type PWM qui permet de gérer le volume
                                       // 100 : minimum / 180 : médium / 255 : maximum
        int      redemarrage   = 0; // 1 => Arret sur bouton valide et prévoir un redemarrage après x seconde (voir dans liste des define)
      };
      
      struct Bouton {
        int      pinPhysique   = 0; // PIN sur laquelle nous lisons la valeur
        int      levelMin      = 0; // Voltage minimum de captation
        int      levelMax      = 0; // Voltage minimum de captation
        ...
        int      cptUp         = 0; // quand > 5 on modifie la valeur  ... permet de filtrer
        int      cptDown       = 0; // quand > 5 on modifie la valeur  ... permet de filtrer
      };

      struct Moteur {
        boolean  sens        = 0; // 0 on avance / 1 on recule
        int      pinSens     = 0;
        int      pinStep     = 0;
      };

      struct Volume {
        int      volGlobale  = 0; // volume en rélle en ml
        int      volCentaine = 0; // chiffre des centaines
        int      volDixaine  = 0; // chiffre des dixaines
        int      volUnite    = 0; // chiffre des unités
        int      volDixieme  = 0; // chiffre des dixièmes
      };
      
      /******************** configuration structure ********************************/
      struct Etat_Global {
        int     etatMoteur     = 0; // 0 => Arrêt, 1 => Perfusion, 2 => Bolus, 3 => Avance rapide, 4 => Recul rapide
        int     prevEtatMoteur = 0; // Etat permettant de mesurer les demandes de changement d'état du moteur
        int     volBolus       = 0;
        int     prevVolBolus   = 0;

        // parametre configurable
        long volumeBolus        = 0; // Volume qui définit la vitesse du bolus => unité dixième de ml
        long volumeBolusMax     = 0; // Volume max autorisé lors d'un bolus => unité dixième de ml
        long volumeAvance       = 0; // 5000ml/h de volume pour avoir une vitesse élevée de moteur => unité dixième de ml
        long volumeRecul        = 0; // 5000ml/h de volume pour avoir une vitesse élevée de moteur => unité dixième de ml
        long adresseLCD         = 0; //0x27   // Adresse de l'écran LCD
        ...
        int valeurA2           = 0;
        int valeurA3           = 0;
        int valeurTension[8]  = {1024,1024,1024,1024,1024,1024,1024,1024};
        int valeurTensionNbre = 8; 
        int valeurTensionPos  = 0; 
        unsigned long valeurTensionLast = 0;
        int valeurTensionMoy  = 1024;
        
        /* Menu/Afficheur */
        int affichageEnCours   = 0; // 0 => Menu, 1 => Réglage de la seringue, 2 => Réglage du volume, 3 ou 10 => Avance ou retour rapide confirmation
        ...
        int choixEnCours       = 0; // 0 => Choix seringue, 1 => Choix volume
        int positionVol        = 0; // 0 => chiffre des centaines, 1 => chiffre des dizaines, 2 => chiffre des unités, 3 => chiffre des dizièmes
        int positionSer        = 0; // Id de la seringue en cours de sélection
        ...
        Bouton etatBatterie;    // Mesure la tension pour vérifier sa bonne santé de la batterie
        Bouton etatSecteur;    // Mesure la tension pour vérifier la connection du chargeur à l'alimentation 220V
        ...
        /* Valeur seringue */
        int    seringueSel    = 0; // seringue sélectionnée
        const char*  seringueList[6] = {"seringue1", "seringue2", "seringue3", "seringue4", "seringue5", "seringue6"};
        const int    seringueDiam[6] = {265        , 262        , 261        , 263        , 264        , 266        };
        int    seringueNbre   = 6; // nombre de seringues dans la liste
        // damietre seringue / ou section
