...
void TimerOne::StopTimer()
{
  TCCR1B &= ~(_BV(CS10) | _BV(CS11) | _BV(CS12));
}


ISR(TIMER1_COMPA_vect)          // interrupt service routine that wraps a user defined function supplied by attach_OCA_Interrupt
{
  Timer1.isr_OCA_Callback();
}
...
void TimerOne::initialize_timer(long period, unsigned char unit)
{
  long period_microsecond = period;
  
  if (unit == TIMER_millisecond) period_microsecond = period*1000;
  if (unit == TIMER_second) period_microsecond = period*1000000;    
  initialize(period_microsecond); 
}
...
void TimerOne::Enable_ISR_OC(char pin)
{
  if(pin == 1 || pin == 9)       TIMSK1 = _BV(OCIE1A);
  else if(pin == 2 || pin == 10) TIMSK1 = _BV(OCIE1B);  
}

void TimerOne::Disable_ISR_OC(char pin)
{
  if(pin == 1 || pin == 9)       TIMSK1 = ~_BV(OCIE1A);
  else if(pin == 2 || pin == 10) TIMSK1 = ~_BV(OCIE1B); 
}
...
{  
  // Specifique pour projet PSE 
  // Pour optimiser le calcul F_CPU est pris en compte dans la fonction de conversion
  // long cycles = (F_CPU / 2000000) * microseconds;                                // the counter runs backwards after TOP, interrupt is at BOTTOM so divide microseconds by 2
  long cycles= microseconds;
...
#ifdef DEBUG_MOTEUR
  DebugMoteur(F(" Prescalaire "));
  DebugMoteurln(clockSelectBits);  
  DebugMoteur(F(" ICR1 "));  
  DebugMoteurln(cycles);  
#endif
}
...
void TimerOne::RestartPwm(char pin)
{
  if(pin == 1 || pin == 9)       TCCR1A &= _BV(COM1A1);   // clear the bit that enables pwm on PB1
  else if(pin == 2 || pin == 10) TCCR1A &= _BV(COM1B1);   // clear the bit that enables pwm on PB2
}
...
void TimerOne::attach_OCA_Interrupt(void (*isr)())
{
  isr_OCA_Callback = isr;                                       // register the user's callback with the real ISR
}

void TimerOne::detachInterrupt()
{
  TIMSK1 &= ~_BV(TOIE1);                                   // clears the timer overflow interrupt enable bit                              // timer continues to count without calling the isr
}
